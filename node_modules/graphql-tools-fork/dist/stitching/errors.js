...
function relocatedError(originalError, nodes, path) {
    if (Array.isArray(originalError.path)) {
        return new graphql_1.GraphQLError(originalError.message, originalError.nodes, originalError.source, originalError.positions, path ? path : originalError.path, originalError.originalError, originalError.extensions);
    }
    return new graphql_1.GraphQLError(originalError && originalError.message, (originalError && originalError.nodes) || nodes, originalError && originalError.source, originalError && originalError.positions, path, originalError);
}
exports.relocatedError = relocatedError;
function slicedError(originalError) {
    return relocatedError(originalError, originalError.nodes, originalError.path ? originalError.path.slice(1) : undefined);
}
...
function getErrorsByPathSegment(errors) {
    ...
    errors.forEach(function (error) {
        ...
        var current = record[pathSegment] || [];
