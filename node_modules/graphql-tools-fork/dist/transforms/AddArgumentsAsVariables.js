...
var AddArgumentsAsVariablesTransform = /** @class */ (function () {
    function AddArgumentsAsVariablesTransform(targetSchema, args, newSchema) {
        this.targetSchema = targetSchema;
        this.args = args;
        this.newSchema = newSchema;
    }
    AddArgumentsAsVariablesTransform.prototype.transformRequest = function (originalRequest) {
        var _a = addVariablesToRootField(this.targetSchema, originalRequest.document, this.args, this.newSchema), document = _a.document, newVariables = _a.newVariables;
...
function addVariablesToRootField(targetSchema, document, args, newSchema) {
            ...
            if (selection.kind === graphql_1.Kind.FIELD) {
                ...
                field.args.forEach(function (argument) {
                    if (argument.name in args) {
                        ...
                        if (newSchema) {
                            newVariables[variableName] = utils_1.transformInputValue(argument.type, args[argument.name], function (t, v) {
                                var newType = newSchema.getType(t.name);
                                return newType ? newType.serialize(v) : v;
                            });
                        }
                        else {
                            // tslint:disable-next-line:max-line-length
                            console.warn('AddArgumentsAsVariables should be passed the wrapping schema so that arguments can be properly serialized prior to delegation.');
